  ...
  return (
    ...
    >
      ...
      <div className="relative w-36 shrink-0 self-stretch bg-black/10">
        ...
        {/* LinkedIn badge pinned bottom-left of photo */}
        {q.linkedin && (
          <a
            href={q.linkedin.startsWith("http") ? q.linkedin : `https://${q.linkedin}`}
            target="_blank"
            rel="noopener noreferrer"
            className="absolute bottom-2 left-2 w-6 h-6 rounded flex items-center justify-center"
      ...
      {/* ── Right: text — all slides stacked so height never jumps ── */}
      <div className="flex-1" style={{ display: "grid" }}>
        {quotes.map((slide, i) => (
          <div
            key={i}
            className="p-5 flex flex-col justify-center gap-1"
            style={{ gridArea: "1 / 1", visibility: i === idx ? "visible" : "hidden" }}
          >
            <p className={`font-bold text-sm leading-tight ${textCls}`}>{slide.name}</p>
            <p className={`text-xs ${mutedCls}`}>{slide.title}</p>
            <p className={`text-sm italic leading-relaxed mt-2 ${textCls}`}>&ldquo;{slide.quote}&rdquo;</p>
          </div>
        ))}
      </div>
